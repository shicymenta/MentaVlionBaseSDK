...
@class MENTADatabase;
@class MENTAStatement;

/** Represents the results of executing a query on an `<MENTADatabase>`.
...
@interface MENTAResultSet : NSObject

@property (nonatomic, retain, nullable) MENTADatabase *parentDB;
...
/** `MENTAStatement` used by result set. */

@property (atomic, retain, nullable) MENTAStatement *statement;

///------------------------------------
/// @name Creating and closing a result set
///------------------------------------

/** Create result set from `<MENTAStatement>`
 
 @param statement A `<MENTAStatement>` to be performed
 
 @param aDB A `<MENTADatabase>` to be used
 
 @return A `MENTAResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(MENTAStatement *)statement usingParentDatabase:(MENTADatabase*)aDB;
