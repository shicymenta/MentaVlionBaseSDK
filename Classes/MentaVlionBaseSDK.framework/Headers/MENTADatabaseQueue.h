...
/** To perform queries and updates on multiple threads, you'll want to use `MENTADatabaseQueue`.

 Using a single instance of `<MENTADatabase>` from multiple threads at once is a bad idea.  It has always been OK to make a `<MENTADatabase>` object *per thread*.  Just don't share a single instance across threads, and definitely not across multiple threads at the same time.

 Instead, use `MENTADatabaseQueue`. Here's how to use it:

 First, make your queue.

    MENTADatabaseQueue *queue = [MENTADatabaseQueue databaseQueueWithPath:aPath];

 Then use it like so:

    [queue inDatabase:^(MENTADatabase *db) {
        [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:1]];
        [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:2]];
        [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:3]];

        MENTAResultSet *rs = [db executeQuery:@"select * from foo"];
        while ([rs next]) {
            //…
        }
    }];
 ...
 An easy way to wrap things up in a transaction can be done like this:
    ...
    [queue inTransaction:^(MENTADatabase *db, BOOL *rollback) {
        [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:1]];
        [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:2]];
        [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:3]];
 ...
 `MENTADatabaseQueue` will run the blocks on a serialized queue (hence the name of the class).  So if you call `MENTADatabaseQueue`'s methods from multiple threads at the same time, they will be executed in the order they are received.  This way queries and updates won't step on each other's toes, and every one is happy.

 ### See also

 - `<MENTADatabase>`

 @warning Do not instantiate a single `<MENTADatabase>` object and use it across multiple threads. Use `MENTADatabaseQueue` instead.
 
 @warning The calls to `MENTADatabaseQueue`'s methods are blocking.  So even though you are passing along blocks, they will **not** be run on another thread.

 */

@interface MENTADatabaseQueue : NSObject
/** Path of database */
...
/** Create queue using path.
 ...
 @return The `MENTADatabaseQueue` object. `nil` on error.
 */
...
// If you need to nest, use MENTADatabase's startSavePointWithName:error: instead.
