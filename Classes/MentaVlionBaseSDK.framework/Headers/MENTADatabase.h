...
NS_ASSUME_NONNULL_BEGIN
    ...
    #define MENTADBAutorelease(__v) ([__v autorelease]);
    #define MENTADBReturnAutoreleased MENTADBAutorelease

    #define MENTADBRetain(__v) ([__v retain]);
    #define MENTADBReturnRetained MENTADBRetain

    #define MENTADBRelease(__v) ([__v release]);

    #define MENTADBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define MENTADBAutorelease(__v)
    #define MENTADBReturnAutoreleased(__v) (__v)

    #define MENTADBRetain(__v)
    #define MENTADBReturnRetained(__v) (__v)

    #define MENTADBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define MENTADBDispatchQueueRelease(__v)
    #else
        #define MENTADBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^MENTADBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);

typedef NS_ENUM(int, MENTADBCheckpointMode) {
    MENTADBCheckpointModePassive  = 0, // SQLITE_CHECKPOINT_PASSIVE,
    MENTADBCheckpointModeFull     = 1, // SQLITE_CHECKPOINT_FULL,
    MENTADBCheckpointModeRestart  = 2, // SQLITE_CHECKPOINT_RESTART,
    MENTADBCheckpointModeTruncate = 3  // SQLITE_CHECKPOINT_TRUNCATE
};

/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 
 ### Usage
 The three main classes in MENTADB are:

 - `MENTADatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<MENTAResultSet>` - Represents the results of executing a query on an `MENTADatabase`.
 - `<MENTADatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<MENTADatabasePool>` - A pool of `MENTADatabase` objects.
 - `<MENTAStatement>` - A wrapper for `sqlite_stmt`.
 
 ### External links
 
 - [MENTADB on GitHub](https://github.com/ccgus/fmdb) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [MENTADB mailing list](http://groups.google.com/group/fmdb)
 - [SQLite FAQ](http://www.sqlite.org/faq.html)
 
 @warning Do not instantiate a single `MENTADatabase` object and use it across multiple threads. Instead, use `<MENTADatabaseQueue>`.
...
@interface MENTADatabase : NSObject
...
/** Create a `MENTADatabase` object.
 
 An `MENTADatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `MENTADatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `MENTADatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    MENTADatabase *db = [MENTADatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    MENTADatabase *db     = [MENTADatabase databaseWithPath:dbPath];
 ...
 @return `MENTADatabase` object if successful; `nil` if failure.
...
/** Create a `MENTADatabase` object.
 ...
 2. `nil`.  An in-memory database is created.  This database will be destroyed with the `MENTADatabase` connection is closed.
...
/** Initialize a `MENTADatabase` object.
...
/** Initialize a `MENTADatabase` object.
 ...
 An `MENTADatabase` is created with a local file URL to a SQLite database file.  This path can be one of these three:
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) MENTADBExecuteStatementsCallbackBlock _Nullable)block;
...
/** Execute select statement
 ...
 Executing queries returns an `<MENTAResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[MENTAResultSet next]>`) from one record to the other.
 ...
 @return A `<MENTAResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see MENTAResultSet
 @see [`MENTAResultSet next`](<[MENTAResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
...
- (MENTAResultSet * _Nullable)executeQuery:(NSString*)sql, ...;
...
- (MENTAResultSet * _Nullable)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (MENTAResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
/** Execute select statement
 ...
 `func executeQuery(sql: String, values: [Any]?) throws  -> MENTAResultSet!`
...
- (MENTAResultSet * _Nullable)executeQuery:(NSString *)sql values:(NSArray * _Nullable)values error:(NSError * _Nullable __autoreleasing *)error;
...
- (MENTAResultSet * _Nullable)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary * _Nullable)arguments;


// Documentation forthcoming.
- (MENTAResultSet * _Nullable)executeQuery:(NSString *)sql withVAList:(va_list)args;
 ...
 */
- (BOOL)checkpoint:(MENTADBCheckpointMode)checkpointMode error:(NSError * _Nullable *)error;
...
/** Performs a WAL checkpoint
 ...
 @return YES on success, otherwise NO.
 */
- (BOOL)checkpoint:(MENTADBCheckpointMode)checkpointMode name:(NSString * _Nullable)name error:(NSError * _Nullable *)error;
...
/** Performs a WAL checkpoint
 ...
 @return YES on success, otherwise NO.
 */
- (BOOL)checkpoint:(MENTADBCheckpointMode)checkpointMode name:(NSString * _Nullable)name logFrameCount:(int * _Nullable)logFrameCount checkpointCount:(int * _Nullable)checkpointCount error:(NSError * _Nullable *)error;
...
+ (NSString*)MENTADBUserVersion;

+ (SInt32)MENTADBVersion;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    MENTAResultSet *rs = [db executeQuery:@"SELECT * FROM employees WHERE RemoveDiacritics(first_name) LIKE 'jose'"];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [MENTADatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one MENTADB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using MENTADatabase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to MENTADB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using MENTADB you will not need to interact directly with `MENTAStatement`, but rather with `<MENTADatabase>` and `<MENTAResultSet>` only.
...
@interface MENTAStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
