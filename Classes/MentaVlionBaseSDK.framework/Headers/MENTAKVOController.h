...
/**
 ...
 MENTAKVOKeyPath(string.length) => @"length"

 Or even the complex case:

 MENTAKVOKeyPath(string.lowercaseString.length) => @"lowercaseString.length".
 */
#define MENTAKVOKeyPath(KEYPATH) \
@(((void)(NO && ((void)KEYPATH, NO)), \
({ const char *mentakvokeypath = strchr(#KEYPATH, '.'); NSCAssert(mentakvokeypath, @"Provided key path is invalid."); mentakvokeypath + 1; })))
...
/**
 ...
 MENTAKVOClassKeyPath(NSString, length) => @"length"
 MENTAKVOClassKeyPath(NSString, lowercaseString.length) => @"lowercaseString.length"
 */
#define MENTAKVOClassKeyPath(CLASS, KEYPATH) \
@(((void)(NO && ((void)((CLASS *)(nil)).KEYPATH, NO)), #KEYPATH))

NS_ASSUME_NONNULL_BEGIN

/**
 Key provided in the @c change dictionary of @c MENTAKVONotificationBlock that's value represents the key-path being observed
 */
extern NSString *const MENTAKVONotificationKeyPathKey;
...
/**
 ...
 @param change The change dictionary which also includes @c MENTAKVONotificationKeyPathKey
 */
typedef void (^MENTAKVONotificationBlock)(id _Nullable observer, id object, NSDictionary<NSString *, id> *change);

/**
 @abstract MENTAKVOController makes Key-Value Observing simpler and safer.
 @discussion MENTAKVOController adds support for handling key-value changes with blocks and custom actions, as well as the NSKeyValueObserving callback. Notification will never message a deallocated observer. Observer removal never throws exceptions, and observers are removed implicitly on controller deallocation. MENTAKVOController is also thread safe. When used in a concurrent environment, it protects observers from possible resurrection and avoids ensuing crash. By default, the controller maintains a strong reference to objects observed.
 */
@interface MENTAKVOController : NSObject
 ...
 */
- (void)observe:(nullable id)object keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options block:(MENTAKVONotificationBlock)block;
 ...
 */
- (void)observe:(nullable id)object keyPaths:(NSArray<NSString *> *)keyPaths options:(NSKeyValueObservingOptions)options block:(MENTAKVONotificationBlock)block;
